...
func processServiceFile(path string, collectors *collectors, configSchemas *[]ServiceConfigSchema) error {
	...
	if err := validateServiceYAML(service); err != nil {
...
func validateServiceYAML(service map[string]any) error {
...
func validateEnumField(fieldName, value string) error {
	...
	if enumDef, exists := serviceSchema.Enums[enumName]; exists {
		if !slices.Contains(enumDef.Values, value) {
...
func generateIndividualConfigFile(structName, filePath string) error {
	...
	content := fmt.Sprintf("// Code generated by generate-services. DO NOT EDIT.\npackage types\n\ntype %s struct {}\n", structName)
	return os.WriteFile(filePath, []byte(content), permFile)
}
