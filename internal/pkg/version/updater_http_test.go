...
func TestCheckForUpdates(t *testing.T) {
	t.Run("returns newer release", func(t *testing.T) {
		// Skip if running as dev build
		if IsDevBuild() {
	...
	t.Run("returns no update for same version", func(t *testing.T) {
		if IsDevBuild() {
			t.Skip("Skipping test in dev build")
		}
	...
	t.Run("skips prerelease versions", func(t *testing.T) {
		checker := NewUpdateChecker("1.0.0")
		checker.client = &http.Client{
			Transport: &mockTransport{
				DoFunc: func(req *http.Request) (*http.Response, error) {
