...
func ResolveUpServices(args []string, cfg *config.Config) ([]servicetypes.ServiceConfig, error) {
	...
	if err := manager.ValidateServices(serviceNames); err != nil {
		return nil, pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "stack", "resolve_services", err)
	}
...
func (s *Service) Start(ctx context.Context, req StartRequest) error {
	...
	if err := s.GenerateComposeFile(req.Project, req.ServiceConfigs); err != nil {
		return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "generate compose file", err)
	}

	// Load project
	project, err := s.project.Load(req.Project)
	if err != nil {
		return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "load", err)
	}
	...
	if err != nil {
		if len(req.ServiceConfigs) > 0 {
			return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "start services", err)
		}
		return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "start", err)
	}
	...
	if err := s.executeLocalInitScripts(ctx, req.ServiceConfigs, req.Project); err != nil {
		return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "execute init scripts", err)
	}
...
func (s *Service) Stop(ctx context.Context, req StopRequest) error {
	...
	if req.Remove {
		...
		if err != nil {
			return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "remove", err)
		}
		return nil
	}
	...
	if err != nil {
		...
		if len(serviceNames) > 0 {
			return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "stop services", err)
		}
		return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "stop", err)
	}
	return nil
}
...
func (s *Service) Logs(ctx context.Context, req LogRequest) error {
	...
	if err != nil {
		return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "get logs", err)
	}
	return nil
}
...
func (s *Service) Exec(ctx context.Context, req ExecRequest) error {
	...
	if err != nil {
		return pkgerrors.NewServiceError(pkgerrors.ErrCodeOperationFail, "project", "exec command", err)
	}
	return nil
}
