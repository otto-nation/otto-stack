...
func TestMergeProjectConfigs(t *testing.T) {
	t.Run("merges configs with local overriding base", func(t *testing.T) {
		base := &config.Config{
...
func TestResolveServiceConfigs(t *testing.T) {
	t.Run("resolves service configs", func(t *testing.T) {
		cfg := &config.Config{
	...
	t.Run("uses provided service names", func(t *testing.T) {
		cfg := &config.Config{
			Stack: config.StackConfig{
				Enabled: []string{"postgres"},
			},
