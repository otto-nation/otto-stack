...
var commandHandlers = map[string]func() cliTypes.CommandHandler{
...
func init() {
	handlers.Register(handlers.PackageServices, func(name string) cliTypes.CommandHandler {
		if factory, exists := commandHandlers[name]; exists {
			return factory()
		}
		return nil
	})
