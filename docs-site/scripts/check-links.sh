...
LINK_CHECK_OUTPUT=$(find content -name '*.md' -exec markdown-link-check --config .mlc_config.json {} \; 2>/dev/null && markdown-link-check --config .mlc_config.json README.md 2>/dev/null)
